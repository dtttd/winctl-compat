...
winctl.GetActiveWindow = () => {
  const win = new Window();
  return win;
};

winctl.GetWindowByClassName = () => {};

winctl.GetWindowByTitleExact = () => {};

winctl.EnumerateWindows = () => {};
...
class WindowEventsEmitter extends EventEmitter {
  ...
  updatePollingLoops() {
    Object.keys(this.eventLoops).forEach(loopName => {
      ...
      var listenerCount = props.events.reduce(
        (prev, curr) => prev + this.listenerCount(curr),
        0
      );
  ...
  checkNewWindow() {
    ...
    winctl
      .FindWindows(win => win.isVisible() && win.getTitle())
      .then(windows => {
